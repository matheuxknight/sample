...
class HTTP_WebDAV_Server 
{
	...
	function serveRequest($uri, $path_info) 
	{
		$this->base_uri = $uri;
		$this->uri      = $uri . $path_info;

		$this->path = $this->_urldecode($path_info);
		if(!strlen($this->path)) $this->path = '/';
        
		if(!$this->_check_if_header_conditions()) return;
		...
		if($method == 'head' && !method_exists($this, 'head')) $method = 'get';
    ...
    function http_PROPPATCH() 
    {
        if ($this->_check_lock_status($this->path)) {
            ...
            echo "<D:multistatus xmlns:D=\"DAV:\">\n";
            echo " <D:response>\n";
            echo "  <D:href>".$this->_urlencode($this->path)."</D:href>\n";
