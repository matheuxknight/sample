...
$sansspacehosts = getdbolist('Sansspacehost', "not sans and license_active order by $order");
...
echo "<th><a style='color: #555;' href='javascript:admin_setorder(\"license_concurrent desc\");'>Conc.</a></th>";
echo "<th><a style='color: #555;' href='javascript:admin_setorder(\"license_total desc\");'>Max</a></th>";
...
foreach($sansspacehosts as $host)
{
	...
	echo "<td>$host->license_concurrent</td>";
	echo "<td>$host->license_total</td>";
...
//			'confirm'=>"Are you sure to delete this host $host->name?"));
	...
	if($host->requestupdate == 1)
		echo mainimg('green-check.png');

	else if($host->requestupdate == 2)
		echo mainimg('16x16_misc.png');

	else
		if($host->version != SANSSPACE_VERSION && controller()->rbac->globalNetwork())
			echo "<a href='javascript:admin_flag($host->id)'>".mainimg('16x16_bottom.png')."</a>";
	
//			echo l(mainimg('16x16_bottom.png'), array('flag', 'id'=>$host->id), array('title'=>'Flag for Update'));
	
//				array('title'=>,
//					'submit'=>'', 'params'=>array('command'=>'flag', 'id'=>$host->id),
//					'confirm'=>"Are you sure to flag for update this host $host->name?"));
