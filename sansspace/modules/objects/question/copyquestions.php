...
showNavigationBar($bank->parent);
showObjectHeader($bank);
showObjectMenu($bank);

echo "<h3>Copy Questions</h3>";
...
JavascriptReady("onShowObjectBrowser(0, false, true,
	'bankid', 'bankid_xx', $bank->id, '$bank->name', bankFileSelected);");
...
{
	$.get('/quiz/bank_results?id='+selectedid, '', function(data)
	{
		$('#bank_results').html(data);
	});
}
